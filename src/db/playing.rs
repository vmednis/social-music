...
impl db::DbInternal {
    ...
    pub async fn set_playing(&mut self, room_id: String, track_id: String, start_time: u128, length: u64) {
        ...
        let _: () = con.hset_multiple(Self::key_playing(room_id), &data[..]).await.unwrap();
    }

    pub async fn get_playing(&mut self, room_id: String) -> Option<Playing> {
        let mut con = self.client.get_async_connection().await.unwrap();
        let data: Option<HashMap<String, String>> = con.hgetall(Self::key_playing(room_id)).await.unwrap();
